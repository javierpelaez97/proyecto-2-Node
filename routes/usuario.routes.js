...
const { buscarPorId, buscarUnoPorMatricula, buscarTodos, crearUsuario, login, buscarTodosPorMail } = require('../controllers/usuario.controller')
...
router.get("/:matricula", async (req, res) => {
    try {
        const usuarioEncontrado = await buscarUnoPorMatricula(req.params.matricula)
        if (usuarioEncontrado) {
            res.json(usuarioEncontrado)
        }
        else {
